...
If adventurers travel long distances they usually rely on mounts to cover my distance. The most common mount would be the \textbf{riding horse}. However, some races require a smaller mount, like a \textbf{pony} or \textbf{mastiff}, to get around and a spellcaster may cast find steed to get his first mount. This section covers the most common basic mounts a adventuring party may start out with.
...
\begin{DndMonster}[float*=b,width=\textwidth + 8pt]{Elk Mount}
  \begin{multicols}{2}
    ...
    The elk opposes disadvantage against one melee attack that would hit it or its rider. To do so, the elk must see the attacker and use his antlers to fend the foe off.

    \DndMonsterAction{Dogfight}
    The elk adds 2 to its AC and the AC of its rider against all melee attacks from one creature that would hit it or its rider. To do so, the elk must see the attacker and use his antlers to fend the foe off.
...
\begin{DndMonster}[float*=b,width=\textwidth + 8pt]{Pony Mount}
  \begin{multicols}{2}
    ...
    The pony rears up, whirling its hooves and inflicting disadvantage on an opponents next attack roll and any attack rolls against it or its rider until the beginning of its next turn.

    \DndMonsterSection{Reactions}

    \DndMonsterAction{Step Out}
    If the pony is approached by an opponent from the rear, meaning a creature enters the space behind it, the rider may use their reaction and lat the pony perform a \textbf{Combat Stomp} as an attack of opportunity.
...
\begin{DndMonster}[float*=b,width=\textwidth + 8pt]{Riding Horse Mount}
  \begin{multicols}{2}
    ...
    \DndMonsterAction{Rear Up (1/Day)}
    The horse rears up, whirling its hooves and inflicting disadvantage on an opponents next attack roll and any attack rolls against it or its rider until the beginning of its next turn.

    \DndMonsterSection{Reactions}

    \DndMonsterAction{Step Out}
    If the horse is approached by an opponent from the rear, meaning a creature enters the space behind it, the rider may use their reaction and lat the horse perform a \textbf{Combat Stomp} as an attack of opportunity.
